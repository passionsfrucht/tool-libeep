...
for f in sys.argv[1:]:
    ...
    print([f"{cnt.get_channel(i)}" for i in range(cnt.get_channel_count())])
    print("  sample frequency: %i" % (cnt.get_sample_frequency()))
    print("  sample count: %i" % (cnt.get_sample_count()))
    print("  samples(0, 2):")
    print([f"{i:0.2f}" for i in cnt.get_samples(0, 2)])
    print("  triggers: %i" % (cnt.get_trigger_count()))
    print([f"{cnt.get_trigger(i)}" for i in range(cnt.get_trigger_count())])
